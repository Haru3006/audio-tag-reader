...
Status read_and_validate_edit_args(char *argv[], EditInfo *editInfo)
{
    ...
    editInfo->tag = tag_by_type(argv[2]);
    ...
    printf("Successfully arguments validated\n");
    return e_success;
}
...
Status copy_header(EditInfo *editInfo)
{
    ...
    printf("successfully copied header \n");
    return e_success;
}
...
Status read_frame_header(char *frame_id, int *size, char *flags, EditInfo *editInfo)
{
    ...
    *size = (size_buffer[0] << 24) | (size_buffer[1] << 16) | (size_buffer[2] << 8)  | (size_buffer[3]);    /* Convert Big Endian to int */
...
Status edit_frames(EditInfo *editInfo)
{
    ...
    while (1)
    {
        ...
        if (frame_id[0] == 0) /* If empty frame (padding), stop */
        {
            break;
        }
    ...
    printf("Frames edited successfully\n");
    return e_success;
}